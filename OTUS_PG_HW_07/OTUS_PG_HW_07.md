...
UPDATE 1
```
первая сессия - накладываем блокировку на первую сессию второй сессией. (теперь первая ждет отрабтки второй)
```
postgres=*# update forlocks set value = value + 5 where id in (2);
```
вторая сессия - накладываем блокировку на вторую сессию третьей сессией (теперь вторая ждет отработки третьей)
```
postgres=*# update forlocks set value = value + 15 where id in (3);
```
третья сессия - накладываем блокировку на третью сессию первой сессией. В итоге получаем зацикленную взаиммоблокировку между тремя мессиями и как результат обрыв одной из них. Получаем автоматический откат транзакции в третьей сессии, чем свидетельствует отказ (после разрешения взаимоблокировки) выполнить commit;  
```
...
7) Смотрим логи с указанием наличия взаимоблокировки. ERROR:  deadlock detected
```
...
## Блокировки без условия where возможны (задание *), если обновлять в двух транзакциях одно и то же поле. Столбец бцдет обновляться полность, посольку обновление будет проводиться без условия where и сследовательно наложится эксклюзивная блокировка на всю таблицу.
...
```
в третьей сесиии смотрел наличие блокировки
```
