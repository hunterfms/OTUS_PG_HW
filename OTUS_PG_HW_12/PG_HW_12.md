...
1) Подготовил две виртуальные машины PG1 и PG2 (psql (14.13 (Ubuntu 14.13-0ubuntu0.22.04.1))), об на версии. Настроил на обоих машинах конфигурацию pg_hba.conf в разделе ipv4 для двустороннего  подключения по 5432. Добавил на обоих машинах в postgresql.conf listen_addresses = 'localhost, ip-подключающейся_машины' и wal_level = logical. перезагрузил СУБД на обоих машинах. Далее запустил 3ю машину PG15 с добавлением её в конфиги pg_hba.conf на PG1, PG2 для подключения к ним с целью репликации таблиц дял записи на PG15.
...
postgres=#
```
11) На третьей машине PG15 создал пустые таблицы с идеинтичной структурой для репликации. Далее создал подписки на одноименные таблицы для записи с PG1 и PG2.  Спустя несколько секунд проверил записи на локальных таблицах (таблицы заполнились средствами репликации с PG1 и PG2)
```
