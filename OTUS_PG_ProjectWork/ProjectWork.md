...
По мере ознакомления со структурой проекта возникли идеи по нормальзации структуры объектов проекта:
1. Было бы эффективнее упразднить таблицу справочник руководителей подразделений department_manager. Данные этой таблицы хоть и позволяют отслеживать историю смены руководителей но такая история уже есть совокупно в таблицах привязки сотрудника к департаменту с таблицей должностей. Иными словами можно выдать историю смены руководителей отдела связанным запросом, а не занимать под эти «результатирующие данные» (сорные)  диски.
2. Было бы эффективнее упразднить таблицу employees.title перенеся поле должности сотрудника и дату его окончания работы на ней в справочник сотрудников employee, поскольку при добавлении сотрудника всегда ему присваивается должность. В такой модели следует создать отдельный справочник должностей, что займет меньше места на диске нежели текущая структура и будет 
...
7) Проверка производительности запроса на неоптимизированной структуре базы с актуальным планом explain (analyze, buffers)
